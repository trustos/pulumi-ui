...
func TestForkNomadClusterRoundTrip(t *testing.T) {
	body, err := os.ReadFile("../../programs/nomad-cluster.yaml")
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	...
	rendered, err := programs.RenderTemplate(yamlBody, cfg)
	if err != nil {
