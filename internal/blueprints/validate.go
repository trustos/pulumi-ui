...
// ValidationError is one structured error produced by ValidateBlueprint.
...
// ValidateBlueprint runs all six validation levels and returns every error
// found. Levels run sequentially; rendering-dependent levels are skipped if
// the template cannot be parsed or rendered.
func ValidateBlueprint(yamlBody string) []ValidationError {
...
func validateAgentAccessContext(rendered string) []ValidationError {
	...
	if !hasAnyCompute {
		return []ValidationError{{
			...
			Message: "agentAccess is enabled but the blueprint has no compute resources — the agent bootstrap has nothing to inject into",
		}}
	}
