<script lang="ts">
  import vcnOnlyTemplate from '$lib/blueprint-graph/templates/vcn-only.yaml?raw';
  import singleInstanceTemplate from '$lib/blueprint-graph/templates/single-instance.yaml?raw';
  import privateSubnetTemplate from '$lib/blueprint-graph/templates/private-subnet.yaml?raw';
  import webServerTemplate from '$lib/blueprint-graph/templates/web-server.yaml?raw';
  import bastionHostTemplate from '$lib/blueprint-graph/templates/bastion-host.yaml?raw';
  import devEnvironmentTemplate from '$lib/blueprint-graph/templates/dev-environment.yaml?raw';
  import databaseServerTemplate from '$lib/blueprint-graph/templates/database-server.yaml?raw';
  import loadBalancedClusterTemplate from '$lib/blueprint-graph/templates/load-balanced-cluster.yaml?raw';
  import haPairTemplate from '$lib/blueprint-graph/templates/ha-pair.yaml?raw';
  import orchestratorClusterTemplate from '$lib/blueprint-graph/templates/orchestrator-cluster.yaml?raw';
  import multiTierAppTemplate from '$lib/blueprint-graph/templates/multi-tier-app.yaml?raw';
  ...
  let {
    ...
    onSelect: (graph: BlueprintGraph) => void;
    onClose: () => void;
    onBlank: () => void;
  } = $props();
  ...
  function countResources(items: BlueprintItem[]): number {
    let count = 0;
  ...
  function totalResources(graph: BlueprintGraph): number {
  ...
  const templates: (TemplateEntry & { graph: BlueprintGraph })[] = [
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        {#if !searchQuery.trim() && !activeCategory}
          ...
          >
            ...
            <p class="text-xs text-muted-foreground mt-1">Empty blueprint with no resources</p>
          </button>
        {/if}
