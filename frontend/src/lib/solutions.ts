...
const nocobaseSolution: SolutionCard = {
  ...
  userFields: [
    ...
    { key: 'domain', label: 'NocoBase Domain', type: 'text', required: true, placeholder: 'nocobase.example.com', description: 'Public domain where NocoBase will be accessible' },
  ],
  deriveConfig: (input) => {
    const baseDomain = input.domain?.includes('.') ? input.domain.split('.').slice(1).join('.') : input.domain;
    return {
      config: { nodeCount: '1' },
      applications: {
        traefik: true,
      ...
      appConfig: {
        ...
        'postgres.pgadminDomain': `pgadmin.${baseDomain}`,
        'nocobase.domain': input.domain || '',
