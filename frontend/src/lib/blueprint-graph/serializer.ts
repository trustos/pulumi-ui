import type { BlueprintGraph, BlueprintSection, BlueprintItem, LoopSource, ConfigFieldDef, OutputDef, VariableDef } from '$lib/types/blueprint-graph';
import { parseArrayValue, parseObjectValue, isArrayOfObjects } from '$lib/blueprint-graph/object-value';
...
function itemsHaveAutoAD(items: BlueprintItem[]): boolean {
...
function cleanStaleDependsOn(item: BlueprintItem, validNames: Set<string>): BlueprintItem {
...
export function graphToYaml(graph: BlueprintGraph): string {
  const lines: string[] = [];
  ...
  function collectNames(items: BlueprintItem[]) {
    for (const item of items) {
...
function serializeItem(lines: string[], item: BlueprintItem, indent: string, loopVar?: string, loopCtx?: LoopContext, serCtx?: SerializeCtx): void {
