...
import type { BlueprintItem, ResourceItem, ConditionalItem, LoopItem } from '$lib/types/blueprint-graph';

function flattenItems(items: BlueprintItem[]): BlueprintItem[] {
...
function findResource(items: BlueprintItem[], name: string): ResourceItem | undefined {
...
const yaml = readFileSync('../blueprints/nomad-cluster.yaml', 'utf-8');
...
describe('config field details', () => {
  ...
  // gets the correct types from BlueprintMeta.configFields via the API.
  ...
  it('config fields preserve group assignments from meta', () => {
    ...
    // API provides authoritative group assignments via BlueprintMeta.configFields.
