...
import type { BlueprintGraph, LoopItem, ResourceItem } from '$lib/types/blueprint-graph';

// ── helpers ────────────────────────────────────────────────────────────────

function makeGraph(items: BlueprintGraph['sections'][0]['items']): BlueprintGraph {
  return {
...
function untilLoop(configKey: string, variable: string, items: BlueprintGraph['sections'][0]['items']): LoopItem {
  return { kind: 'loop', variable, source: { type: 'until-config', configKey }, serialized: false, items };
}

function listLoop(variable: string, values: string[], items: BlueprintGraph['sections'][0]['items']): LoopItem {
...
describe('@auto roundtrip', () => {
  ...
  function makeGraphWithAdCount(items: BlueprintGraph['sections'][0]['items']): BlueprintGraph {
    return {
      ...makeGraph(items),
