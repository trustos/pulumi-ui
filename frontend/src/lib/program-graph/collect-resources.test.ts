...
function listLoop(variable: string, values: string[], items: ProgramItem[]): LoopItem {
  return {
    kind: 'loop',
    variable,
...
function untilLoop(variable: string, configKey: string, items: ProgramItem[]): LoopItem {
  return {
    kind: 'loop',
    variable,
...
function cond(items: ProgramItem[], elseItems?: ProgramItem[]): ConditionalItem {
  return {
    kind: 'conditional',
...
describe('showNetworkingWarning logic', () => {
  ...
  function shouldShowWarning(items: ProgramItem[]): boolean {
