<script lang="ts">
  ...
  import { getBlueprint, createBlueprint, updateBlueprint, validateBlueprint, forkBlueprint } from '$lib/api';
  import { getOciSchema } from '$lib/schema';
  import { buildWarnByType, type WarnEntry } from '$lib/blueprint-graph/schema-utils';
  import { graphToYaml } from '$lib/blueprint-graph/serializer';
  import { yamlToGraph } from '$lib/blueprint-graph/parser';
  import { insertAgentAccess, removeAgentAccess } from '$lib/blueprint-graph/agent-access';
  import { scaffoldNetworkingGraph, scaffoldNetworkingYaml, hasNetworkingResources } from '$lib/blueprint-graph/scaffold-networking';
  import { propagateRename, propagateRenameYaml } from '$lib/blueprint-graph/rename-resource';
  import { collectAllResources, getMissingAgentOutputs, COMPUTE_RESOURCE_TYPES } from '$lib/blueprint-graph/collect-resources';
  import { getGraphExtras } from '$lib/blueprint-graph/resource-defaults';
  import type { BlueprintGraph, BlueprintSection } from '$lib/types/blueprint-graph';
  ...
  let graph = $state<BlueprintGraph>({
  ...
  // Recursively update a named resource's property at propIndex in a BlueprintGraph.
  function updatePropertyValue(
    ...
    function updateItems(items: import('$lib/types/blueprint-graph').BlueprintItem[]): import('$lib/types/blueprint-graph').BlueprintItem[] {
  ...
  onMount(async () => {
    ...
    try {
      if (fork) {
        const result = await forkBlueprint(name);
        const yaml = result.blueprintYaml;
        ...
        const p = await getBlueprint(name);
        const yaml = (p as any).blueprintYaml ?? '';
        programName = p.name;
  ...
  function scheduleValidation() {
    ...
    validationTimer = setTimeout(async () => {
      ...
      try {
        const result = await validateBlueprint(yamlText);
  ...
  // ── Template gallery ──────────────────────────────────────────────────────
  function selectTemplate(template: BlueprintGraph) {
    graph = template;
  ...
  function collectAllResourceNames(items: import('$lib/types/blueprint-graph').BlueprintItem[]): string[] {
    const names: string[] = [];
  ...
  function collectVisualErrors(
    items: import('$lib/types/blueprint-graph').BlueprintItem[],
    path: string,
    ...
    function check(items: import('$lib/types/blueprint-graph').BlueprintItem[], path: string) {
      for (const item of items) {
  ...
  async function save() {
    ...
    if (!yaml.trim()) {
      saveError = 'Blueprint YAML is empty.';
      return;
    }
    ...
    try {
      if (isNew || fork) {
        await createBlueprint({ name: programName.trim(), displayName, description, blueprintYaml: yaml });
      } else {
        await updateBlueprint(name, { displayName, description, blueprintYaml: yaml });
      }
      navigate('/blueprints');
    } catch (e) {
      saveError = e instanceof Error ? e.message : String(e);
...
{#if showGallery}
  <BlueprintTemplateGallery
    onSelect={selectTemplate}
    onBlank={startBlank}
    onClose={() => navigate('/blueprints')}
  />
{/if}
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <Tooltip.Root>
        <Tooltip.Trigger
          ...
          onclick={() => navigate('/blueprints/docs')}
        >Docs</Tooltip.Trigger>
        <Tooltip.Content>Open the YAML Blueprint Reference</Tooltip.Content>
      </Tooltip.Root>
      <Button variant="outline" onclick={() => navigate('/blueprints')}>Cancel</Button>
      <Button onclick={save} disabled={saving}>{saving ? 'Saving...' : 'Save Blueprint'}</Button>
    </div>
  </div>
