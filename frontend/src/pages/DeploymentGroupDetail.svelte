<script lang="ts">
  ...
  async function load() {
    loading = true;
    try {
      [group, accounts] = await Promise.all([getGroup(id), listAccounts()]);
    } catch {
      group = null;
    } finally {
      loading = false;
  ...
  function getAccountName(accountId: string | null): string {
    ...
    return accounts.find(a => a.id === accountId)?.name ?? accountId.slice(0, 8) + '...';
  }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            } else if (ev.type === 'output' || ev.type === 'complete') {
...
<div class="max-w-3xl mx-auto">
  ...
  {:else if !group}
    <div class="text-center py-12 text-muted-foreground">Group not found.</div>
  {:else}
    <div class="flex items-center justify-between mb-6">
      <div>
        <button class="text-sm text-muted-foreground hover:text-foreground mb-2" onclick={() => navigate('/')}>
          ← Dashboard
        </button>
        ...
        <div class="flex items-center gap-2 mt-1">
          ...
          <span class="text-sm text-muted-foreground">{group.members.length} accounts</span>
        </div>
      </div>
      <div class="flex gap-2">
        ...
        {/if}
        <Button variant="destructive" onclick={() => { deleteOpen = true; }}>Delete Group</Button>
      </div>
    </div>
    ...
    <Card.Root class="mb-6">
      ...
      <Card.Content>
        <div class="flex items-center gap-2 flex-wrap">
          {#each group.members.sort((a, b) => a.order - b.order) as member, i}
            <button
              class="flex items-center gap-2 p-3 border rounded hover:bg-muted/50 transition-colors text-left"
              onclick={() => navigate(`/stacks/${encodeURIComponent(member.stackName)}`)}
            >
              <Badge variant={member.role === 'primary' ? 'default' : 'secondary'} class="text-[10px]">
                {member.role}
              </Badge>
              <div>
                <p class="text-sm font-mono">{member.stackName}</p>
                <p class="text-xs text-muted-foreground">
                  {getAccountName(member.accountId)} · {getAccountRegion(member.accountId)}
                </p>
              </div>
            </button>
        ...
        {#each group.members as member}
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <span class="font-mono text-sm">{member.stackName}</span>
            </div>
...
<Dialog.Root bind:open={deleteOpen}>
  <Dialog.Content class="max-w-sm">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Delete the group <strong>{group?.name}</strong>? The individual stacks will remain and can be managed independently.
      </Dialog.Description>
    </Dialog.Header>
    <Dialog.Footer>
