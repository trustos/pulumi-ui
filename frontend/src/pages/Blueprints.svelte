<script lang="ts">
  ...
  import type { BlueprintMeta, ValidationError } from '$lib/types';
  import { listBlueprints, getBlueprint, createBlueprint, updateBlueprint, deleteBlueprint, validateBlueprint } from '$lib/api';
  ...
  let editingName = $state(''); // empty = new blueprint
  ...
  async function openEdit(name: string) {
    ...
    try {
      ...
      editorYAML = (p as any).blueprintYaml ?? '';
      editorOpen = true;
      runValidation(editorYAML);
    } catch (e) {
      error = e instanceof Error ? e.message : String(e);
  ...
  async function saveBlueprint() {
    ...
    validating = true;
    try {
      const result = await validateBlueprint(editorYAML);
      validationErrors = result.errors ?? [];
      validationPassed = result.valid;
      if (!result.valid) {
    ...
    try {
      if (editingName) {
        await updateBlueprint(editingName, {
          displayName: editorDisplayName,
          description: editorDescription,
          blueprintYaml: editorYAML,
        });
      } else {
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <Button variant="outline" onclick={openNew}>New Blueprint (YAML)</Button>
...
<Dialog.Root bind:open={editorOpen}>
  <Dialog.Content class="max-w-4xl max-h-[90vh] flex flex-col">
    <Dialog.Header>
      <Dialog.Title>{editingName ? 'Edit Blueprint' : 'New Blueprint'}</Dialog.Title>
      <Dialog.Description>
      ...
      <div class="space-y-1 flex-1">
        <div class="flex items-center justify-between">
          <label class="text-sm font-medium" for="prog-yaml">
            Blueprint YAML <span class="text-destructive">*</span>
          </label>
          <label
    ...
    <Dialog.Footer class="flex items-center justify-between gap-4">
      ...
      <div class="flex gap-2 shrink-0">
        ...
        <Button onclick={saveBlueprint} disabled={editorSaving || validating || validationErrors.length > 0}>
          {editorSaving ? 'Saving...' : 'Save Blueprint'}
        </Button>
      </div>
    </Dialog.Footer>
  </Dialog.Content>
